...
xml_path = 'two_pendulums.xml' #xml file (assumes this is in the same folder as this file)
...
#https://mujoco.readthedocs.io/en/latest/XMLreference.html#actuator-general
...
def controller(model, data):
    ...
    if (data.qpos[0]>=2.5 and FSM==FSM_SWINGUP):
        FSM =FSM_HOLD
    if (FSM==FSM_SWINGUP):
        set_velocity_servo(2,0.1)
        data.ctrl[2]=1
    
    if (FSM==FSM_HOLD):
        set_position_servo(1,80)
...
# cam.elevation = -45
# cam.distance = 2
# cam.lookat = np.array([0.0, 0.0, 0])p
cam.azimuth = 87.39186256525785 ; cam.elevation = -20.56361102720078 ; cam.distance =  4.596936047021883
cam.lookat =np.array([ 0.0 , 0.0 , 1.0 ])

angle_start = 0.9
data.qpos[0] = angle_start
data.qpos[2] = angle_start
...
while not glfw.window_should_close(window):
    ...
    # while (data.time - time_prev < 1.0/60.0): #Normal speed
    while (data.time - time_prev < 1.0/240.0): #Slowed down
